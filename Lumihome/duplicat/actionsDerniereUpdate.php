...
// ============================================================
// ANTI-ABUS — limite de gains de points par type d'action/heure
// Le keyword est ce qui est cherché dans la colonne "detail" du points_log
// ============================================================
const ANTI_ABUS = [
    'connexion' => ['max_par_heure' => 1,  'keyword' => 'connexion'],
    'toggle'    => ['max_par_heure' => 4,  'keyword' => 'lumage'],   // allumage / extinction
    'intensite' => ['max_par_heure' => 4,  'keyword' => 'ntensit'],  // Intensité
    'couleur'   => ['max_par_heure' => 4,  'keyword' => 'ouleur'],
    'add'       => ['max_par_heure' => 3,  'keyword' => 'Ajout'],
    'delete'    => ['max_par_heure' => 3,  'keyword' => 'uppression'],
];
...
if ($action === 'login') {
    ...
    try {
        ...
        // Connexion récompensée 1x/heure seulement
        if (!estLimite($pdo, $user['id'], 'connexion')) {
...
if ($action === 'add_light') {
    ...
    try {
        ...
        if (estLimite($pdo, $uid, 'add'))
            jsonResponse(['ok' => false, 'limite' => true,
                'msg' => '⏳ Trop rapide ! Maximum 3 ajouts par heure.']);
...
if ($action === 'toggle_light') {
    ...
    try {
        ...
        $ptsGagnes = 0;
        if ($oldVal !== $etat && !estLimite($pdo, $uid, 'toggle')) {
...
if ($action === 'update_intensite') {
    ...
    try {
        ...
        $ptsGagnes = 0;
        if (!estLimite($pdo, $uid, 'intensite')) {
...
if ($action === 'update_couleur') {
    ...
    try {
        ...
        $ptsGagnes = 0;
        if ($ancienne !== $col && !estLimite($pdo, $uid, 'couleur')) {
...
/**
 * Vérifie si l'utilisateur a dépassé sa limite horaire pour ce type d'action.
 * Retourne true = limité (pas de points accordés).
 */
function estLimite(PDO $pdo, int $uid, string $typeAction): bool {
    $cfg = ANTI_ABUS[$typeAction] ?? null;
    if (!$cfg) return false;
    ...
    $st = $pdo->prepare("
        ...
        WHERE id_user = ?
          ...
          AND detail LIKE ?
    ");
    $st->execute([$uid, '%' . $cfg['keyword'] . '%']);
    return (int)$st->fetchColumn() >= $cfg['max_par_heure'];
}
