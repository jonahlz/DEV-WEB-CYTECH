...
try {
    ...
    // Données de niveau pour le widget de progression
    $st_niv = $pdo->prepare('SELECT u.points, u.niveau, u.pts_connexion, u.pts_actions,
        nc.couleur_hex, nc.emoji AS niv_emoji, nc.libelle AS niv_libelle
        FROM utilisateurs u
        LEFT JOIN niveaux_config nc ON nc.niveau = u.niveau
        WHERE u.id = ?');
    $st_niv->execute([$uid]);
    $niv_user = $st_niv->fetch();

    // Prochain niveau
    $niv_suivant = $pdo->prepare('SELECT * FROM niveaux_config WHERE pts_requis > ? ORDER BY pts_requis ASC LIMIT 1');
    $niv_suivant->execute([$niv_user['points'] ?? 0]);
    $prochain_niv = $niv_suivant->fetch();
} catch(Exception $e) {
...
<div class="page">
  ...
  <?php if ($niv_user): 
    ...
    $proch_pts = $prochain_niv ? (float)$prochain_niv['pts_requis'] : $pts;
    // Trouver pts du niveau actuel pour la progression
    $niveaux_seuils = ['debutant'=>0,'intermediaire'=>3,'avance'=>5,'expert'=>7];
    $pts_niveau_actuel_seuil = $niveaux_seuils[$niv_user['niveau']] ?? 0;
    $range_niv = $prochain_niv ? ($proch_pts - $pts_niveau_actuel_seuil) : 1;
    $prog_niv  = $prochain_niv ? min(100, round(($pts - $pts_niveau_actuel_seuil) / $range_niv * 100)) : 100;
  ?>
  <div class="card" style="margin-bottom:1.5rem;padding:1rem 1.5rem;
    ...
    <div style="display:flex;align-items:center;gap:.85rem">
      ...
      <div>
        <div style="font-size:.72rem;color:var(--muted);font-weight:600;
                    text-transform:uppercase;letter-spacing:.4px">Votre niveau</div>
        <div style="font-family:'Syne',sans-serif;font-weight:800;
                    color:<?= escape($col) ?>;font-size:1.1rem">
    ...
    <div style="flex:1;min-width:180px">
      ...
      <div style="display:flex;justify-content:space-between;
        ...
        <span>→ <?= escape($prochain_niv['emoji'].' '.$prochain_niv['libelle']) ?>
          (<?= $prochain_niv['pts_requis'] ?> pts)</span>
      </div>
      <div style="height:7px;background:var(--surface2);border-radius:4px;overflow:hidden">
        <div style="height:100%;width:<?= $prog_niv ?>%;
                    background:<?= escape($col) ?>;border-radius:4px;transition:width 1s"></div>
      </div>
      <?php else: ?>
