...
try {
    $pdo = db();
    $st = $pdo->prepare("
        SELECT id, login, nom, prenom, email, role, sexe, date_naissance, type_membre, points, created_at
        FROM utilisateurs
        WHERE id = ?
        LIMIT 1
    ");
...
if ($erreur_profil !== '') {
    echo '<div class="page"><div class="card" style="border-color:rgba(239,68,68,.35);color:#fca5a5;">' . escape($erreur_profil) . '</div></div>';
    return;
}
...
$points = isset($profil['points']) ? number_format((float)$profil['points'], 2, ',', ' ') : '0';
$created_at = !empty($profil['created_at']) ? date('d/m/Y H:i', strtotime($profil['created_at'])) : '-';
?>

<div class="page">
    <div style="margin-bottom:1.75rem;">
        <h1 style="font-family:Syne,sans-serif;font-size:1.9rem;font-weight:800;margin-bottom:.2rem;">Mon profil</h1>
    ...
    <div class="profil-grid" style="display:grid;grid-template-columns:1.1fr .9fr;gap:1rem;">
        <div class="card">
            ...
            <div style="margin-top:1.25rem;padding-top:1rem;border-top:1px solid var(--border);display:flex;gap:.75rem;flex-wrap:wrap;">
                <button class="btn btn-primary" onclick="toggleEditProfil()">Modifier mon profil</button>
                <a class="btn btn-outline" href="/Lumihome/index.php?page=dashboard">Retour au dashboard</a>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div style="display:grid;gap:.8rem;">
                <div style="padding:.85rem 1rem;background:var(--surface2);border:1px solid var(--border);border-radius:10px;">
    ...
    <div id="profil-edit-card" class="card" style="margin-top:1rem;display:none;">
        <div style="margin-bottom:1rem;">
            ...
            <div style="font-size:.82rem;color:var(--muted);margin-top:.15rem;">
                Laissez les champs mot de passe vides si vous ne voulez pas le changer.
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input class="form-input" id="p-mdp" type="password" placeholder="Laisser vide pour ne pas changer">
            </div>

            <div class="form-group">
                <label class="form-label">Confirmer le mot de passe</label>
                <input class="form-input" id="p-mdp2" type="password" placeholder="Répéter le nouveau mot de passe">
            </div>
        </div>
